...
Verify Product Transactions
    ...
    Run Keyword And Continue On Failure  Get OM SNS Event
    ...
    Run Keyword And Continue On Failure  Perform Netcracker Validations For Pending State
    Submit Sale To VPS
    ...
    Run Keyword And Continue On Failure  Wait For Order State To Be Updated    ${happy_path_order_id}    Closed
...
Suite Setup
    ...
    Set Suite Variable  ${VPS_PAYMENT_METHOD}   SEPA
...
Upsert Order To OM
   ...
   [Tags]   OM  captureResponseTime
    ...
    &{location}   createDictionary  addressLines=${ADDRESS_LINE}  countryCode=${COUNTRY_CODE}  city=${CITY}  regionOrState=${STATE}  zipOrPostCode=${POSTAL_CODE}
    ${status}   ${message}   Run Keyword And Ignore Error   Variable Should Exist  ${LATITUDE}
    ...
    Set Suite Variable    ${happy_path_order_id}
...
Get OM SNS Event
    ...
    ${start_state}    ${end_state}    ${message}    Run Keyword And Continue On Failure    Get Event From OM SNS    ${happy_path_order_id}
