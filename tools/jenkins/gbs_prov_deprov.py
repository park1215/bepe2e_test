...
def testModem(modem,paramSets):
      ...
      for paramSet in paramSets:
         ...
         outputFile = "output_provision_"+modem+"_"+planString+".xml"
         output = robot.run("../../demo/demo_provision_verify.robot",exitonfailure=True,critical="critical",output=outputFile,xunit="x_"+outputFile)
         ...
         outputFile = "output_deprovision_"+modem+"_"+planString+".xml"
         output = robot.run("../../demo/demo_disconnect_verify.robot",exitonfailure=True,critical="critical",output=outputFile,xunit="x_"+outputFile)
         # replace suite names in output.xml with suite+suffix
         replaceSuiteNamesXunit("x_"+outputFile,suiteSuffix)
         replaceSuiteNames(outputFile,suiteSuffix)
         print("result for "+outputFile+" = "+str(output))
...
def main(argv):
   ...
   try:
      ...
      print('python gbs_prov_deprov.py --config <config_file> --version <versionname> --cycle <cyclename>')
      sys.exit(2)
   for opt, arg in opts:
      if opt == '-h':
        print('python gbs_prov_deprov.py --config <config_file> --version <versionname> --cycle <cyclename>')
